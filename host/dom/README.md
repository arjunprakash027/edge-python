...
**Parsing JSON returns.** Several handlers return JSON strings; parse them with the [`json`](https://github.com/dylan-sutton-chavez/edge-python/tree/main/std/json) standard package. It isn't built-in, declare it via a `packages.json` alias or import it by URL, as the examples below do.
