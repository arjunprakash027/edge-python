use crate::abi::{
    ...
    DecodeBits, EncodeRequest, Op, PrimitiveBytes, TAG_INVALID,
};
...
use super::{
    error_stash, get_val, handles, put_val, with_recv, with_vm,
};
...
/* IterNext: pops the head of a list-handle and returns it; on empty,
...
/// its bytes into the caller-provided buffer `dst[..dst_max]`.
#[unsafe(no_mangle)]
