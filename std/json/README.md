...
Pre-built `.wasm` is served from `https://cdn.edgepython.com/std/json.wasm`, deployed by the `Std` workflow in the `edge-python` repo. Host runtimes resolve the URL via their cache (browser worker auto-caches in IndexedDB; other hosts can mirror).
