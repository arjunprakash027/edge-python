...
- **Context managers**: `with` and `async with` invoke `__enter__` / `__exit__` on the context-manager value; a truthy return from `__exit__` suppresses the raised exception, matching CPython.
...
Edge Python is **functional-first**. Classes exist as basic state containers, not as the primary abstraction. Single-level inheritance, `super()`, and dunder dispatch are supported where they pay their way, but behaviour reuse is expected to go through function composition rather than deep class hierarchies.
